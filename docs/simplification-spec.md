...
| D13 | Custom PIN and reset | `0000` works initially. A custom coach PIN replaces it, so `0000` is no longer a bypass. A signed-in coach can reset the PIN in Settings. | Stage 3 setting/reset contract; Stages 4–5 settings, prepared-device exit and recovery. |
| D14 | Backdated training raffle round | Training newly entered from paper belongs to the current raffle round at record creation, regardless of attendance date. A session already recorded offline keeps its original assigned round when it later syncs, even after a reset; never silently move it to the new round. | Stage 3 round validation; Stage 4 creation/outbox; Stage 6 raffle. |
| D15 | Player retirement and restoration | Retirement hides a player from future attendance lists while preserving their identity, recorded attendance, hours and earned raffle tickets. A coach can restore the player later. Retirement alone does not remove current-round ticket eligibility; Start fresh still changes eligibility by round without deleting history. | Stage 3 retire/restore operations; Stage 4 roster; Stages 5–6 lookup and reporting. |

D13–D15 were explicitly approved by the owner on 2026-09-23. These supersede older statements that PIN replacement/reset, backdated-round assignment and player retirement are undecided. Approval does not extend to sub-team retirement, deleting players, changing historical expected lists or the other remaining recommendations.

Recommended PIN details for the prototype: four digits to avoid jersey-number collisions; changing the PIN replaces the default exit code rather than leaving `0000` as a bypass. Settings outside kiosk allow the signed-in coach to change/reset it. Persist the coach preference and make the active exit verifier available through interrupted connectivity. PIN length, recovery presentation and storage design remain implementation recommendations; replacement and signed-in reset are approved by D13.
...
| P07 | Retirement, retained current-round tickets and restoration are now approved in D15. Retire a player from the active roster instead of scrubbing history. Retired players retain existing current-round tickets; exclusion from future draws, if desired, is a separate explicit action and outside the basic removal flow. |
| P08 | Implement approved D04 with a small **Change date** action beside **Today** on attendance, after session entry (practice expectation confirmation under D10, direct entry for training). Keep that chosen date across resume/sync and show it in the finish summary. Use date-only local-calendar semantics; retain raw legacy dates. Date edits never move existing raffle eligibility between rounds. D14 now approves the current round at creation for newly entered backdated training and preserving the original round for previously recorded offline sessions. |
...
| A06 | Kiosk handles one/multiple/no matches, distinct `0`/`00`, repeated entry, correct undo, cleared input and exit without ending session. Default `0000` and coach-set PIN flows work; under approved D13, `0000` stops working after a custom PIN is set and a signed-in coach can reset the PIN in Settings. | 2, 5 |
