...
## Owner decisions D13–D15 and exact continuation — 2026-09-23

The owner explicitly approved all three recommended policies after reviewing Claude's continuation at `2a5d8c3`:

- **D13:** `0000` works initially; a custom PIN replaces it. A signed-in coach can reset the PIN in Settings.
- **D14:** training newly entered from paper uses the current raffle round at record creation regardless of its attendance date. A session already recorded offline retains its original round when it syncs after a reset. Do not reassign queued attendance to a new round.
- **D15:** retirement hides players from future attendance lists and preserves identity, attendance, hours and earned tickets. Coaches can restore retired players. Retirement alone does not remove current-round raffle eligibility.

These decisions supersede the corresponding open-policy statements in earlier dated entries. Sub-team retirement, saved expected-list correction, re-dating completed sessions, excused absence and unexpected-attendee streak treatment remain outside this approval. The previous implementation/testing record remains historical evidence, not proof that these newly approved paths are implemented.
...
## Stage 4 coach attendance and prepared-device outbox — 2026-09-24

**Code commit:** `2a0bcd7` (`Implement isolated Stage 4 coach attendance and offline outbox`). Work is on `claude/dazzling-sagan-hxrwfp`. The isolated test project identity was checked before database work: `viouquduxutuslafiooy`. Production `pwgqwcvultxihntvaewo` was neither queried nor changed. `vercel.json` still disables deployment for this branch. No merge, deployment, production migration, legacy migration 008/010, account purge, or Stage 5/6 implementation occurred. Stage 3 SQL remains a tested candidate, not a release migration.

### Exact changes

- Added the Stage 4 test-only entry at `stage4.html` and `src/stage4/`. It uses the published owner-scoped read tables and versioned initialize, session, roster, correction, and settings RPCs. The legacy app entry and secured admin backend are unchanged. `vite.config.ts` requires the exact test-project URL and a test publishable key in `stage4-test` mode, with no credential fallback. `.env.stage4-test.example` documents the required values; the real `.env.stage4-test.local` stays ignored. Run locally with `npm run stage4:dev` or build with `npm run stage4:build`; open `/stage4.html`.
- Added a versioned owner-keyed IndexedDB record containing prepared roster, memberships, teams, current round, settings, the PIN verifier, session snapshots and an ordered operation queue. A single transaction saves each visible change with its immutable operation ID, device ID/sequence, session ID, base revision and payload. Unknown storage versions block access without deleting work. The app caches its HTML, built assets and service worker, verifies the cached files on reopen, and gates new starts on preparation. An update that changes the asset fingerprint visibly requires re-preparation while retaining pending sessions.
- Added Start Practice and Optional Training, the selected-team/All Kaizen expected union, Expected and Other player views, attendance mark/undo, backdated paper entry, multiple memberships, roster/team create and edit, player retire/restore with the same ID, completed-session attendance corrections, and online PIN set/reset. New sessions credit 1.5 hours; loaded historical durations stay intact. The cached PIN verifier is ready for Stage 5 kiosk use. A local finish allows the next session even while earlier operations wait.
- Added ordered replay and a five-second retry while online. Acknowledgment removes only the exact queue head. `40001`, `23505`, `55000`, `22023` and `22P02` retain intended work as conflicts; auth errors pause upload, show a same-coach sign-in action, and keep the queue. Sign-out with pending changes has an explicit in-page warning. Isolated browser test controls can simulate failed local writes, network loss, a lost RPC response and an expired auth check.
- Added seven IndexedDB/workflow tests covering transaction failure, owner separation, unsupported storage version, immutable queue order, A/B finished plus C active, identical retry after a lost response, conflict retention and auth ownership.

### Verification actually run

- `npx tsc --noEmit`, `npm run build`, `npm run stage4:build`, `git diff --check`, and all seven Stage 4 tests passed. The default build was checked only; its generated `dist/` files were restored, so no build artifact or legacy entry change is in the commit.
- In the in-app browser against the isolated test project, prepared online, stopped the local server, reopened the page without that server, and saw the cached coach UI return to **Ready offline** after validating files. The browser test control then blocked Supabase requests; two completed sessions survived reopen and synchronized on reconnect. A separate A/B/C sequence had A and B completed locally, C active after reopen, and all queued operations delivered in order. Test SQL showed one row per session, 1.50 hours per new session, no practice tickets, and exactly one ticket for a completed training with one present player.
- Verified selected-team/All Kaizen deduplication with a shared player (three unique players across two teams), `0` versus `00`, unexpected attendance, mark then undo, a backdated training dated 2026-09-20 bound to the current round, and retirement/restoration of Kayla under the same player UUID with history and an earned ticket retained. A completed-training correction changed one present player to absent and removed its derived ticket without changing the session date or round. Set a custom PIN, refreshed its verifier from the backend, and reset to `0000`.
- Simulated one local write failure: the card remained absent and the UI showed a save error. Simulated a lost RPC acknowledgment after server processing: the queued `finish_v1` retried, and test SQL showed one finish ledger row and revision 1. Simulated an expired auth check: upload paused with pending work, then resumed after same-coach sign-in. Signed out with a pending A operation, signed in as coach B on the same browser, saw no A session or queue, then returned as A and delivered it. Rebuilt the app while A work was pending: new starts were blocked as unprepared, but the pending session remained visible. A browser viewport check at 820 × 1180 showed the tablet layout.
- Full `npx vitest run`: **62 passed, 3 failed, one collection failure** (65 collected tests; 8/11 files passed). The same legacy failures were reproduced from untouched remote baseline `447f72a` in a separate `/tmp` checkout: two `KioskScreen.test.tsx` assertions, one dated `LaunchPage.test.tsx` time-preset assertion, and `stats.test.ts` collection without `VITE_SUPABASE_URL`. Baseline was 55 passed and the identical three failures plus collection failure. These failures are not caused by this Stage 4 change.

### Remaining verification and limits

- Browser network controls and stopping a local preview are simulations, not a physical iPad airplane-mode pass. No actual iPad, iPadOS storage pressure, installed-home-screen lifecycle, or Safari eviction behavior was tested. The test entry was not hosted or deployed. The Stage 3 source-derived test schema still needs the separate production-compatibility release gate already documented above. The browser test accounts and synthetic roster/session data remain in the isolated test project for repeatable checks; no real roster was imported.
- The 401 test used a one-shot simulated expired response. Same-coach reauthentication and queue preservation were observed; a natural JWT expiry over time still needs device testing. Likewise, the lost-response control demonstrated idempotent retry on the test backend but physical radio interruption remains untested. Browser tests did not exercise a round reset while an offline session waited; the Stage 3 role suite covers that server contract, and the client displays `needs_round_review` when returned.
- Physical-iPad checklist before claiming device reliability: serve the isolated Stage 4 build over a secure test origin; sign in as a synthetic coach; prepare and confirm Ready offline; close the tab or installed app completely; enable airplane mode; reopen without an existing session; start practice, mark and undo, finish A and B, start C, and close/reopen again; check cached PIN behavior once Stage 5 kiosk exists; reconnect and verify A/B/C states, exact 1.5-hour credits and single training tickets in the test backend; repeat with a revoked/expired sign-in, an interrupted response, low-storage/quota failure, and an app update while work is pending. Do not clear Safari site data while work is queued.

### Physical iPad follow-up — 2026-09-24

- The owner ran the guided Coach A test in iPad Safari and explicitly confirmed that Practice C remained active after reopening the page in Airplane Mode. After reconnecting, the app reported that all saved changes had synced. The owner then finished C. This is a user-reported physical-device observation; no iPad logs or screen recording were captured.
- A read-only check of isolated test project `viouquduxutuslafiooy` found the three new Coach A sessions: Practice A completed with two present, Optional Training B completed with one present, and Practice C completed with one present. Each has `credit_hours = 1.50` and exactly one `finish_v1` operation. The ticket-entitlement view has one ticket for B and none for the practices. This confirms server delivery and no duplicate finish or ticket for this run. Production was not queried or changed.
- Follow-up commits `9fb1e1b` and `15a4a13` simplified test sign-in to a Coach A/B picker with short test-only passwords and removed a duplicate sync-success message. The local HTTPS preview was corrected to serve the Stage 4 build. The remaining device checks are natural auth expiry, interrupted radio response, storage pressure or Safari eviction, and an app update while changes are pending. Stage 5 kiosk/PIN interaction and the Stage 3 production-compatibility gate remain open.

### Precise Stage 5 handoff

Build kiosk presentation on the same active `OwnerData` session and call `markPresent`/`enqueue` for check-in and undo. Persist the kiosk/session binding in the owner record before showing the kiosk; restore it after offline reopen. Use exact player-wide string jersey lookup (`0` and `00` distinct), show all matching cards with labels and current present state, and require a card tap even for one match. Hide coach controls in kiosk. Use `verifyPin` against the prepared verifier; `0000` works only in default mode, a custom PIN replaces it, and exit returns to the same active session without finishing it. If the session has ended elsewhere, show the finished-session message rather than coach controls. Exercise local-write failure, same-device offline reopen, keyboard/focus/touch behavior, and real iPad airplane mode. Do not add raffle drawing or analytics here; those remain Stage 6. Keep the Stage 3 release gate and no-deploy guard intact.

## Stage 4 review fixes — 2026-09-24

Branch `claude/dazzling-sagan-hxrwfp`, starting at reviewed head `267f731`. This pass addressed the seven offline recovery and sync findings in the isolated Stage 4 entry. It did not start Stage 5, change the Stage 3 candidate SQL, query or change production `pwgqwcvultxihntvaewo`, load migration 008 or 010, alter the sole super-admin rule or secured admin backend, merge, deploy, or run a production migration. The branch no-deploy guard remains in place.

### Changes and coach resolution choices

- The device remembers the last signed-in owner ID. If auth refresh fails on offline reopen, that owner's prepared data opens with upload paused. Start, mark, and finish remain available. The coach signs in as the same owner after reconnecting to upload. Explicit sign-out clears the owner pointer, the local auth session, and the prepared PIN cache, including when offline; queued attendance stays owner-keyed for later sign-in.
- A blocked queue head now shows its saved intent beside a freshly fetched affected server state. For `40001` or `55000`, the coach can explicitly resend on the displayed server revision with a new operation ID; dependent operations for that same revisioned record shift their base revisions and remain ordered. The coach can instead confirm discarding the blocked operation and operations that depend on it; the affected local records are replaced from the server. A `23505` operation-ID reuse is never automatically given a new ID. Its intent stays for review and can be explicitly discarded. Server refresh remains available with a blocked queue and preserves queued local changes.
- PIN set/reset now use a separate, saved online request. A lost response leaves the exact operation ID and payload for an explicit retry. The cache changes only from an accepted `{mode, revision}` plus the submitted verifier; a stale revision reloads the server PIN state without touching attendance. Successful attendance sync refreshes `{mode, revision, verifier}`. The settings page offers retry or a confirmed review/discard of an unresolved PIN request.
- RPC errors retain the HTTP status. HTTP 5xx, `PGRST000`–`PGRST003`, `08*`, `40P01`, `57014`, `53300`, and `57P0*` wait for retry with the same ID; HTTP 401 and `PGRST301`/`PGRST303` pause for auth. `42501` goes to review. Contract conflicts and validation errors retain the saved intent.
- Session child reads filter by the loaded session IDs and page in 500-row batches. Preparation pages players, teams, and memberships. Local create/rename rejects an active team name with the same trimmed, case-folded spelling. The isolated browser controls can now force a one-shot 503 or `40001`, and can expire the stored test token before an offline reopen. The prepared shell version is 10.

### Verification actually run

- `npx tsc --noEmit`: passed.
- `npx vitest run src/stage4`: 20/20 passed in four files. New cases cover expired-token offline owner recovery and Start Practice, offline sign-out followed by reload, another coach's auth event unable to take over saved work, conflict review/resend and ordered later operations, explicit discard after operation-ID reuse, transient 503 retry, 401 auth pause, 42501 review, pending PIN lost-response retry and stale revision, team-name collisions, server refresh with blocked work, HTTP status preservation, and 1,205-row session and preparation reads.
- `npm run stage4:build`: passed. `git diff --check`: passed. Generated `dist/` files were restored/removed before commit and are not included.
- Opened the local Stage 4 preview in the in-app browser and observed the isolated test sign-in screen. Live signed-in browser scenarios are pending a synthetic Coach A credential or signed-in preview session. The new expired-token, offline sign-out, forced-503, dropped-PIN-response, and resolved-conflict flows have **not** been claimed as live browser passes. No physical iPad check or test-project SQL verification was performed in this pass.

### Signed-in isolated browser follow-up — 2026-09-25

The owner signed into a local Stage 4 test preview as synthetic Coach A. The initial Vite development preview could not complete preparation because `/stage4-assets.json` returned HTML; the signed-in tab was reloaded from the actual `stage4-test` build at the same local origin. Preparation then reached **Ready offline**. This preview serves only the build compiled with the exact isolated test-project guard; no deployment occurred.

- The browser's one-shot `PGRST002`/HTTP 503 control was armed before starting a new Optional Training. Its start first appeared as a saved local change and then reached **Synced**. A synthetic mark and finish also synced. This is a browser simulation of a temporary failure, not a physical network test.
- The one-shot `40001` control was armed before marking a player in a new Practice. The browser displayed **Review required**, kept the local present mark, and showed the server's active session at revision 0 with zero present next to the local intent. Finishing locally left two changes queued. **Refresh prepared data** succeeded while preserving both queued changes and the conflict. The coach's explicit **Resend this change on current revision** action then cleared the queue; the UI reported **All saved changes synced**.
- Read-only SQL in test project `viouquduxutuslafiooy` found both sessions from those two runs completed at revision 2. Each has one accepted `start_v1`, `set_present_v1`, and `finish_v1` ledger entry in device-sequence order. The rejected conflict did not create an accepted ledger entry. This corroborates delivery for these two runs; it does not by itself establish every physical-device condition.

- The owner submitted a synthetic custom PIN after the browser's dropped-acknowledgment control was armed. The browser showed a failed fetch and kept the pending PIN request. The owner selected **Retry PIN request** once; the page then showed **PIN changed and synced** and **Custom four-digit PIN active**. Read-only test-project SQL found one accepted `set_exit_pin_v1` ledger entry for operation `046e8ff2-e66a-43f1-bb75-715dc8251728`, sequence 7, with custom mode at revision 3. Retrying produced no duplicate entry for that operation. A later read found a second, distinct PIN-set request at sequence 8 and revision 4; its submission was not observed in this browser run. The current server PIN mode is custom at revision 4.
- The browser test control expired the stored test token and blocked Supabase requests. After reload, the saved Coach A data opened **Ready offline** with **Sync paused** and **Upload paused**. A new Optional Training was started, Kayla was marked present, and the session was finished while offline. History showed one present and **Saved on this device · waiting to sync**. Sign-out warned that three pending changes would remain on the device; after confirmation and another offline reload, only the sign-in screen appeared, with no coach data.
- After the owner signed back in as Coach A, the saved device record still had the simulated offline flag. Restoring the test network in Settings let all three queued changes sync. Read-only test-project SQL showed one accepted `start_v1`, `set_present_v1`, and `finish_v1` in sequences 9–11 for the same session, completed at revision 2 with one present player, 1.50 credit hours, and one training ticket. **Refresh prepared data** then returned the page to **Ready offline** and showed the server's active custom PIN. No pending PIN retry remained in this browser.
- The first offline reload briefly showed the sign-in screen while the expired-session check was unresolved, then recovered the saved owner. Startup now reads the remembered owner synchronously and pauses upload immediately. A new component test holds the auth check unresolved and verifies that prepared attendance still opens. Component tests are now 4/4; `npx tsc --noEmit` and `npm run stage4:build` passed after this adjustment. The browser test used a simulated expired token and blocked requests; natural JWT expiry and a fresh physical-iPad pass remain untested.

## Stage 5 kiosk check-in — 2026-09-25 (in progress)

The owner authorized the Stage 5 kiosk work after the Stage 4 handoff. This implementation remains in the isolated `stage4-test` entry and test project `viouquduxutuslafiooy`; it does not add a production migration, deploy, merge, raffle drawing or Stage 6 analytics. The branch no-deploy guard and the Stage 3 production-compatibility release gate remain in place.

- The active session can enter kiosk only after its session ID is committed to the owner-keyed device record. Kiosk reopening reads that binding and hides the coach navigation, roster, settings and finish action. Exit clears the binding only after local PIN verification and returns to the same unfinished attendance session. A remotely completed bound session shows **Session finished — ask your coach**; online kiosk refresh also reloads the current exit-code verifier. A forgotten offline custom PIN keeps the kiosk bound, with a visible reconnect, same-coach sign-in and reset/refresh recovery path.
- Kiosk lookup compares the complete jersey-number string, preserving `0` and `00`. It searches the active prepared roster, shows every player with the number and team labels, and requires a card tap even for one result. Check-in and undo use the Stage 4 durable desired-state operation. The screen waits for the local transaction before showing confirmation; a failed write shows an error and no success message. Number input, keypad, backspace, clear, Enter submission, touch-size buttons, no-match guidance and PIN exit are present. The shared number pad accepts one to three digits for jersey lookup and four digits for coach exit; an incorrect PIN clears the entry and keeps kiosk active. Default `0000` works only in default mode; a custom verifier rejects it.
- `npx tsc --noEmit`, `npx vitest run src/stage4` (31/31), `npm run stage4:build` and `git diff --check` passed. Tests cover binding persistence and exit without finish, exact lookup and multiple matches, four-digit keypad exit and failed PIN feedback, custom PIN rejection of `0000`, local-write failure during both check-in and exit, remote finish with pending work retained, and refreshing a reset PIN. The shell cache version was raised to `stage5-shell-11`, so a device must prepare the new assets online before being marked ready for a new offline session.
- In the isolated in-app browser, the upgraded build prepared to **Ready offline**. A synthetic Practice entered kiosk; `0` found only Kayla and `00` found only Owen. Kayla required a card tap, showed a saved-device confirmation, and synced; a repeated lookup showed **Checked in · Undo check-in**, and undo saved and synced. `999` showed no-match guidance. Stopping the local preview server and reloading restored the kiosk and its same session from the cached shell. The preview server was restored afterward. After the owner found that the keypad stopped at three digits, the revised preview showed the four-digit coach exit path. The owner entered the custom PIN in that preview and confirmed that it returned to coach attendance. Physical iPad airplane mode remains untested for Stage 5.
- Read-only SQL in the isolated test project confirmed one `start_v1` and two `set_present_v1` operations (present then absent) at device sequences 12–14 for that Practice. Its server row remains active at revision 2 with zero present players and 1.50 hours per attendee. Kiosk entry/exit create no server attendance operation of their own.
