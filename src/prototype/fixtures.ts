...
function p(
  ...
  return { id, firstName, number, subTeamId, guest: false, retired: false, ...extra };
}
...
const FULL_ROSTER: Player[] = [
  ...
  p("pl-16", "Elena", null, null, { guest: true, legacyName: "Elena (guest)" }),
  p("pl-17", "Tomas", "33", null, { guest: true }),
...
function snapshot(ids: string[], roster: Player[]): Record<string, string | null> {
  const map: Record<string, string | null> = {};
  for (const id of ids) {
    const found = roster.find((x) => x.id === id);
    map[id] = found ? found.subTeamId : null;
  }
  return map;
}
...
export const SCENARIOS: Scenario[] = [
  {
    ...
    blurb: "Four sub-teams, shared jersey numbers, duplicate first names, guests and history.",
  },
  {
    id: "noSubTeams",
  ...
  },
  {
    id: "empty",
    name: "Empty roster",
    blurb: "A new coach with nobody on the roster yet.",
  },
];
...
export function buildScenario(scenario: ScenarioId): PrototypeState {
  ...
  if (scenario === "empty") {
    return { ...base, scenario, players: [], subTeams: [], sessions: [] };
  }
  ...
  if (scenario === "legacy") {
    ...
    return {
      ...
      players: legacyNames.map((name, i) =>
        p(`lg-${i + 1}`, name, null, null, { legacyName: name }),
      ),
      sessions: [],
    };
  }
  ...
  if (scenario === "noSubTeams") {
    return {
      ...
      scenario,
      subTeams: [],
      players: FULL_ROSTER.map((x) => ({ ...x, subTeamId: null })),
  ...
  return {
    ...
    players: FULL_ROSTER.map((x) => ({ ...x })),
    sessions: FULL_SESSIONS.map((s) => ({ ...s })),
  };
}
