...
export default function CoachApp() {
  ...
  async function authenticate(coach: keyof typeof TEST_COACH_EMAIL, secret: string): Promise<void> {
    const result = await client.auth.signInWithPassword({ email: TEST_COACH_EMAIL[coach], password: secret });
    ...
    setAuthPaused(false);
    setStatus('Signed in. Prepare this iPad while online, or resume saved work.');
  }
  async function signIn(event: React.FormEvent) {
    event.preventDefault(); setBusy(true); setStatus('Signing in…');
    try { await authenticate(testCoach, password); setPassword(''); }
