...
import { AUTH_STORAGE_KEY, clearDeviceSignIn, lastOwner, rememberOwner } from './deviceAuth';
...
export default function CoachApp() {
  ...
  async function signIn(event: React.FormEvent) {
    ...
    try {
      ...
      if (result.error) setStatus(result.error.message); else if (lastOwner() && lastOwner() !== result.data.user.id) { await client.auth.signOut({ scope: 'local' }); setStatus('Sign in as the coach whose work is stored on this device, or sign out explicitly first.'); } else { explicitLogout.current = false; rememberOwner(result.data.user.id); setOwnerId(result.data.user.id); setAuthPaused(false); setStatus('Signed in. Prepare this iPad while online, or resume saved work.'); }
    } catch (error) { setStatus(`Sign-in failed: ${message(error)}`); }
    finally { setBusy(false); }
  }
  async function logout() {
    if (data?.queue.length && !logoutWarning) { setLogoutWarning(true); return; }
    try { if (ownerId) await changeOwner(ownerId, current => { current.prepared = null; delete current.pendingPin; return current; }); }
  ...
  async function prepareDevice() {
    ...
    if (syncInFlight.current) { setStatus('Wait for the current upload, then refresh.'); return; }
    syncInFlight.current = true; setBusy(true); setStatus('Saving app shell and roster…');
    try { const next = data.queue.length ? await refreshKeepingQueue(ownerId) : await prepare(ownerId, await readOwner(ownerId)); setData(next); setVerifiedShell(next.prepared!.shellAssets); setStatus(data.queue.length ? 'Server data refreshed. Saved changes and review items remain on this device.' : 'Ready for offline attendance on this device.'); }
  ...
  return <div className="coach-app"><header><div><strong>Kaizen Tracker</strong><small>Coach attendance · test project</small></div><span className={`readiness ${ready ? 'ready' : ''}`}>{ready ? 'Ready offline' : 'Prepare online'}</span></header>
    ...
    {authPaused && <section className="coach-panel warning"><h2>Upload paused</h2><p>Offline attendance remains available. Sign in as this same coach when connected to upload.</p><form onSubmit={signIn}><label>Test coach<select value={testCoach} onChange={e => { setTestCoach(e.target.value as keyof typeof TEST_COACH_EMAIL); setPassword(''); }}><option value="a">Coach A</option><option value="b">Coach B</option></select></label><label>Password<input type="password" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} required /></label><button disabled={busy || !navigator.onLine || !!data.testOffline}>Sign in and resume upload</button></form></section>}
    {data.queue.some(op => op.status !== 'pending') && <section className="coach-panel warning"><h2>Review required</h2>{data.queue.filter(op => op.status !== 'pending').map(op => <div key={op.id}><p>{op.kind} · {op.error}. Your intended change remains stored on this device.</p><p>Local intent: {JSON.stringify(op.payload)}</p><p>Server: {op.serverReview?.summary ?? 'Refresh the server state to compare.'}</p><button disabled={busy || !navigator.onLine || !!data.testOffline} onClick={() => void act(() => reviewBlocked(ownerId, op.id), 'Server state loaded for review.')}>Refresh server state</button>{op === data.queue[0] && <><button disabled={busy || !op.serverReview || !['40001', '55000'].includes(op.errorCode ?? '')} onClick={() => void act(() => resendBlocked(ownerId, op.id), 'New request saved on the current server revision.')}>Resend this change on current revision</button><button disabled={busy || !navigator.onLine || !!data.testOffline} onClick={() => { if (window.confirm('Discard this change and all saved changes that depend on it? This cannot be undone.')) void act(() => discardBlocked(ownerId, op.id), 'Selected saved changes discarded after confirmation.'); }}>Discard change and dependent changes</button></>}</div>)}</section>}
