...
export const SHELL_VERSION = 'stage4-shell-9';
...
export type Operation = { id: string; deviceId: string; sequence: number; kind: string; sessionId: string | null; baseRevision: number; payload: Record<string, unknown>; status: 'pending' | 'conflict' | 'failed'; error?: string };
export type OwnerData = { version: number; ownerId: string; deviceId: string; nextSequence: number; prepared: Prepared | null; sessions: Session[]; queue: Operation[]; lastSyncAt: string | null; testOffline?: boolean };
